...
from Handlers.help_functions import (
    create_start_markup,
    create_lessons_items_markup,
    create_random_lessons_items_markup,
    create_learned_lessons_markup,
    create_lessons_topics_markup, handle_cancel_button,
)
    ...
    try:
        ...
        if lesson_topic.lower() == "cancel":
            ...
            bot.send_message(
                chat_id,
                "Choose a lesson: ",
                reply_markup=create_random_lessons_items_markup(chat_id),
            )
            ...
            bot.send_message(
                chat_id,
                "Choose a lesson: ",
                reply_markup=create_lessons_items_markup(chat_id, lesson_topic),
            )
            bot.register_next_step_handler(message, process_lesson_item, bot)
...
def process_lesson_item(message, bot):
    try:
        ...
        if lesson_by_item:
            ...
            bot.send_message(
                chat_id,
                "No lessons on the selected topic were found.",
...
def handle_answer_lesson(message, lesson_item, lesson_id, bot):
    try:
        ...
        if message.text.lower() == "cancel":
            ...
            bot.send_message(
                chat_id,
                f"Lesson <b>{lesson_item}</b> learned 🎉!",
                parse_mode="HTML",
