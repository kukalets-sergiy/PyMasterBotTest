...
from Handlers.help_functions import create_yes_or_no_markup, handle_cancel_button, create_start_markup
...
def change_code_task_function(bot, message):
    try:
        ...
        bot.send_message(
            chat_id,
            f"Enter the <b>ID</b> number of the code_task you want to change:",
            parse_mode="HTML",
        )
...
def process_code_task_id(message, bot):  # Add bot as a parameter
    try:
        ...
        if not bot_db.get_code_task_by_id(code_task_id):
            bot.send_message(
                chat_id,
                f"Could not find the question with {code_task_id} ID. Cancelled",
            )
