...
from Handlers.help_functions import (
    create_levels_markup,
    create_premium_markup,
    comment_markup,
    create_account_markup,
    create_lessons_topics_markup, create_help_markup,
)
...
def callback_query_handler(call, telebot_instance, inline_keyboard, reply_keyboard):
    ...
    if call.data == "/check_code":
        ...
        telebot_instance.send_message(
            chat_id=call.message.chat.id,
            text=f"For detailed information click on the " f"'Account statistics' button or click 'Cancel' to exit",
    ...
    # Handle the "HELP" button separately
        ...
        telebot_instance.answer_callback_query(callback_query_id=call.id, text="This command does not exist!")
