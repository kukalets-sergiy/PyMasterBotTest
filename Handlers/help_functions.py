    ...
    try:
        if telebot_instance:
            for index in range(1, 3):
                # deletes the previous two messages in the chat in which the current message was received
...
# def handle_cancel_button(bot, chat_id, message_id):
#     """Handle the 'cancel' button press."""
#     bot.send_message(chat_id=chat_id, text="Operation canceled", reply_markup=ReplyKeyboardRemove())
...
def create_tasks_topics_markup(user_id, level_name):
    ...
    markup.add(
        types.KeyboardButton("Choose a task at a different level"),
...
def create_lessons_topics_markup():
    ...
    markup.add(
        types.KeyboardButton("Choose a lesson regardless of the topic"),
        types.KeyboardButton("Cancel"),
    )
...
def create_code_tasks_topics_markup(user_id, level_name):
    ...
    markup.add(
        types.KeyboardButton("Choose a coding task regardless of the topic"),
        types.KeyboardButton("Cancel"),
    )
...
def comment_markup():
    ...
    markup.add(btn_view_comments, btn_write_comment, cancel_button)
...
def create_help_markup():
    ...
    btn_go_site = types.KeyboardButton("Go to Python site")
    btn_cancel = types.KeyboardButton("Cancel")
    reply_markup.add(btn_go_site, btn_cancel)
