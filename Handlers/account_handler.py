...
from Handlers.visual_lesson_representation_handler import (
    progress_lesson_visual_round_repr_function,
...
def account_function(message, bot):
    try:
        ...
        if message_text == "Account statistics":
            ...
            bot.send_message(
                chat_id,
                "Continue to develop actively by taking tests and gaining "
                "knowledge of coding in the Python language!",
            )

        elif message_text.lower() == "cancel":
            bot.send_message(chat_id, "Cancelled.", reply_markup=ReplyKeyboardRemove())
