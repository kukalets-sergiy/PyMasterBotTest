...
from Handlers.help_functions import look_at_added_code_task, handle_cancel_button, create_start_markup
...
def add_easy_code_task_function(bot, message):
    ...
    try:
        ...
        bot.register_next_step_handler(message, process_topic, code_task_id, "easy", bot)  # "easy" level argument
...
def add_middle_code_task_function(bot, message):
    ...
    try:
        ...
        # Ask the user for the code_task topic
        bot.send_message(chat_id, "Enter the code_task topic:")
        bot.register_next_step_handler(message, process_topic, code_task_id, "middle", bot)  # "middle" level argument
...
def add_hard_code_task_function(bot, message):
    ...
    try:
        ...
        bot.register_next_step_handler(message, process_topic, code_task_id, "hard", bot)  # "hard" level argument
...
def process_topic(message, code_task_id, level, bot):
    ...
    try:
        ...
        if message.text == "cancel":
            handle_cancel_button(bot, chat_id, message_id)
        ...
        bot.register_next_step_handler(message, process_question, code_task_id, topic, level, bot)  # Pass level
...
def process_right_answer(
    ...
    try:
        ...
        bot.send_message(
            chat_id,
            "push the button and look what you have added",
            reply_markup=look_at_added_code_task(),
        )
        bot.register_next_step_handler(
            message,
            process_look_at_added_code_task,
            code_task_id,
            topic,
            question,
            first_answer,
...
def process_look_at_added_code_task(
    message,
    code_task_id,
    topic,
    question,
    ...
    try:
        ...
        if message.text in ("Continue", "cancel"):
            ...
            bot.send_message(
                chat_id,
                f"{code_task_id}, {topic}, {question}, {first_answer}, {second_answer}, "
                f"{third_answer}, {right_answer}, {level}",
            )
        else:
            bot.send_message(
                chat_id,
