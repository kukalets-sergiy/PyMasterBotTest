...
from Handlers.visual_code_representation_handler import (
    progress_code_testing_visual_repr_function,
    progress_code_level_visual_repr_function,
    user_visual_code_repr_function,
    progress_code_theory_tests_repr_function,
)
from database.py_master_bot_database import PyMasterBotDatabase2
from Handlers.help_functions import (
    create_yes_or_no_markup,
    delete_previous_messages,
    create_start_markup,
    create_premium_markup,
    create_code_tasks_topics_markup, handle_cancel_button,
)
...
def process_code_task_level(message, bot):
    try:
        ...
        if bot_db.check_this_level_code_task_exists(level_name=level_name):
            message_text = (
                f"Select a test topic of level <b>'{level_name}'</b>"
            ...
            if level_name not in premium_options:
                ...
                bot.send_message(
                    chat_id,
                    f"You will have the opportunity to take CODE test tasks of the level "
                    f"<b>'{level_name}'</b> "
...
def process_code_task_topic(message, level_name, bot):
    try:
        ...
        if code_task_topic.lower() == "cancel":
            ...
            code_task_topic in bot_db.get_code_tasks_topics_by_level(level_name)
            or code_task_topic == "Choose a coding task regardless of the topic"
        ):
...
def choose_code_task_function(message, level_name, code_task_topic, bot):
    try:
        ...
        if len(code_tasks_id) == 0:
            bot.send_message(
                chat_id,
                f"You have already passed all tests of <b>'{level_name}'</b> level "
...
def handle_answer(message, code_task_id, right_answer, level_name, code_task_topic, bot):
    try:
        ...
        if count < 1:
            ...
            bot.send_message(
                chat_id,
                f"Continue testing by level <b>'{level_name}'</b>?\nSelect:",
                parse_mode="HTML",
                reply_markup=create_yes_or_no_markup(),
            )
