...
from Handlers.help_functions import handle_cancel_button, create_start_markup
...
def process_topic(message, bot):  # Add bot as a parameter
    ...
    try:
        ...
        bot.register_next_step_handler(message, process_item, topic, bot)  # Use the original_message
...
def process_text(message, topic, item, description, bot):
    ...
    try:
        ...
        bot.register_next_step_handler(message, process_status, topic, item, description, text, bot)
...
def process_status(message, topic, item, description, text, bot):
    ...
    try:
        ...
        if status not in ["free", "paid", "bonus"]:
            bot.send_message(
                chat_id,
                "Invalid status. Please enter either 'free', 'paid', or 'bonus'.",
            )
            return
