...
def help_request_handler(message, telebot_instance):
    ...
    try:
        ...
        if user_input == "Go to Python site":
            ...
            telebot_instance.send_message(message.chat.id, "If you still have questions, you can enter them here:",
                                          reply_markup=ReplyKeyboardRemove())
            return

        if user_input == "Cancel":
            handle_cancel_button(telebot_instance, chat_id, message_id)
            telebot_instance.send_message(chat_id, "Cancelled.", reply_markup=create_start_markup())
            return
