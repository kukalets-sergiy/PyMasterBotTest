...
from Handlers.visual_representation_handler import (
    progress_testing_visual_repr_function,
    progress_level_visual_repr_function,
    user_visual_repr_function,
    progress_theory_tests_repr_function,
)
...
from Handlers.help_functions import (
    ...
    create_tasks_topics_markup, handle_cancel_button,
)
    ...
    try:
        ...
        if bot_db.check_this_level_task_exists(level_name=level_name):
            ...
            if level_name not in premium_options:
                ...
                bot.send_message(
                    chat_id,
                    f"You will have the opportunity to take test tasks of the level "
                    f"<b>'{level_name}'</b> "
    ...
    try:
        ...
        if task_topic.lower() == "cancel":
            ...
            return
              ...
              task_topic == "Choose a task at a different level"):
...
def choose_test_task_function(message, level_name, task_topic, bot):
    try:
        ...
        def tasks_by_topic(level, topic_task):
            if task_topic == "Choose a task at a different level":
