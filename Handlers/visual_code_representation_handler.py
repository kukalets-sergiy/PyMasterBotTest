    ...
    try:
        ...
        for i, (level, values) in enumerate(user_coding_progress.items()):
            ...
            sizes = [
                ...
                (remaining_code_tasks if remaining_code_tasks > 0 else 0),
            ]
            ...
            def form_labels():
                if completed_code_tasks == 0:
                    ...
                    return (
                        f"{level} ({completed_code_tasks}/{max_value})",
                        f"\n\n\n\n\n\n\n\n\n" "Completed\nenough\ntasks\nof the\n" f"{level}\nlevel",
                    )
    ...
    try:
        ...
        if total_value != 0:
            ...
            for key, value in user_coding_progress.items():
                message_text += (
                    f"at the {key} level: {len(value)},    " f"{'{:.2f}%'.format(len(value) / total_value * 100)}\n"
                )

        message_text += (
            f"\nYour rank by the number of completed CODE tests 💭 " f"<b>{bot_db.check_rank(chat_id).upper()}</b>"
        )
...
def user_visual_code_repr_function(message, bot):
    try:
        ...
        top_users_by_score = [
            (user.id, user.name, user.username, user.score) for user in bot_db.top_users_by_score(top_number=5)
        ]  # number of persons in the rating
        ...
        bot.send_message(
            chat_id,
            f"You have <b>{current_user.status}</b> status until "
