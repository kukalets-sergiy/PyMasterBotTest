...
from Handlers.help_functions import comment_range_button_markup, create_start_markup, handle_cancel_button
from database.py_master_bot_database import PyMasterBotDatabase2


# def process_comments(bot, message, chat_id):
def process_comments(bot, message):
    chat_id = message.chat.id
    message_id = message.message_id  # Отримайте ідентифікатор повідомлення
    try:
...
def process_comments_handler(bot, message):
    try:
        ...
        if user:
            ...
            bot.register_next_step_handler(message, process_name, comment_id, name, bot)
...
def process_comment(message, comment_id, name, bot):
    try:
        ...
        if bot_db.get_comment_by_text(comment):
            bot.send_message(
                chat_id,
                "Comment with the same text already exists. " "Please try again with a different comment.",
            )
            return
...
def next_comments_markup(message, bot):
    try:
        ...
        if message.text == "cancel":
            ...
            bot.reply_to(
                message,
                f"First comments. If there are comments available, "
