...
from Handlers.add_ai_test_tasks_handler import (
    add_ai_test_tasks_function,
    add_ai_code_tasks_function,
    add_ai_lessons_function,
)
from Handlers.add_test_task_by_level_handler import (
    add_easy_test_task_function,
    add_middle_test_task_function,
    add_hard_test_task_function,
)
from Handlers.add_code_task_by_level_handler import (
    add_easy_code_task_function,
    add_middle_code_task_function,
...
# from Handlers.add_test_task_handler import add_ai_tasks_function
...
from Handlers.csv_handler import (
    handle_csv_lessons,
    handle_csv_ai_test_tasks,
    handle_csv_ai_code_tasks,
    handle_csv_ai_lesson_tasks,
)
...
class Bot:
    ...
    async def initialize_database(self):
        """
        Ініціалізує базу даних та створює всі необхідні таблиці.
        """
    ...
    async def start_handler(self, message):
        ...
        if not bot_db.check_user_exists(message.chat.id):
            await bot_db.add_user(message.chat.id, message.chat.first_name, message.chat.username)
            await self.bot.send_message(
                message.chat.id,
                f"Welcome to my bot, {message.chat.first_name}!",
                reply_markup=self.inline_keyboard.get_keyboard(),
            )
        else:
            await self.bot.send_message(
                message.chat.id,
                f"Welcome back, {message.chat.first_name}!",
                reply_markup=self.inline_keyboard.get_keyboard(),
            )

    async def add_lesson_handler(self, message):
        """
        This method adds a lesson
        """
        if message.text.find("/add_lesson") != -1:
            print(self.bot_processor.get_current_mode())
            await add_lesson_function(self.bot, message)

    async def add_admin_handler(self, message):
        """
        This method adds an admin
        """
        if message.text.find("/add_admin") != -1:
            print(self.bot_processor.get_current_mode())
            await add_admin_function(self.bot, message)

    async def add_test_task_handler(self, message):
        """
        This method adds a test_task
        """
        if message.text.find("/add_test_task") != -1:
            await add_test_task_function(self.bot, message)

    async def add_ai_lesson_handler(self, message):
        """
        This method adds lessons from the ai_lessons.csv file
        """
        if message.text.find("/add_ai_lesson_tasks") != -1:
            await add_ai_lessons_function(self.bot, message)

    async def add_ai_test_tasks_handler(self, message):
        """
        This method adds tasks from the ai_test_tasks.csv file
        """
        if message.text.find("/add_ai_test_tasks") != -1:
            await add_ai_test_tasks_function(self.bot, message)

    async def add_ai_code_tasks_handler(self, message):
        """
        This method adds tasks from the ai_code_tasks.csv file
        """
        if message.text.find("/add_ai_code_tasks") != -1:
            await add_ai_code_tasks_function(self.bot, message)

    async def add_test_task_by_level_handler(self, message):
        """
        This method adds a test_task based on the level.
        """

        if message.text.find("/add_easy_test_task") != -1:
            await add_easy_test_task_function(self.bot, message)
        elif message.text.find("/add_middle_test_task") != -1:
            await add_middle_test_task_function(self.bot, message)
        elif message.text.find("/add_hard_test_task") != -1:
            await add_hard_test_task_function(self.bot, message)

    async def change_test_task_handler(self, message):
        """
        This method changes a test_task
        """
        if message.text.find("/change_test_task") != -1:
            await change_test_task_function(self.bot, message)

    async def add_code_task_handler(self, message):
        """
        This method adds a code_task
        """
        if message.text.find("/add_code_task") != -1:
            await add_code_task_function(self.bot, message)

    async def add_code_task_by_level_handler(self, message):
        """
        This method adds a code_task based on the level.
        """

        if message.text.find("/add_easy_code_task") != -1:
            await add_easy_code_task_function(self.bot, message)
        elif message.text.find("/add_middle_code_task") != -1:
            await add_middle_code_task_function(self.bot, message)
        elif message.text.find("/add_hard_code_task") != -1:
            await add_hard_code_task_function(self.bot, message)

    async def change_code_task_handler(self, message):
        """
        This method changes a test_task
        """
        if message.text.find("/change_code_task") != -1:
            await change_code_task_function(self.bot, message)

    async def add_level_handler(self, message):
        """
        This method adds a lesson
        """
        if message.text.find("/add_level") != -1:
            await add_level_function(self.bot, message)

    async def send_error_message(self, error_message):
        await self.bot.send_message(os.getenv("OWNER_CHAT_ID"), f"Error in bot:\n\n{error_message}")

    async def csv_tables_names_lessons(self, message):
        if message.document.file_name == "lessons.csv":
            await handle_csv_lessons(self.bot, message, message.document)
        elif message.document.file_name == "test_tasks.csv":
            await handle_csv_test_tasks(self.bot, message, message.document)
        elif message.document.file_name == "ai_test_tasks.csv":
            await handle_csv_ai_test_tasks(self.bot, message, message.document)
        elif message.document.file_name == "ai_code_tasks.csv":
            await handle_csv_ai_code_tasks(self.bot, message, message.document)
        elif message.document.file_name == "ai_lesson_tasks.csv":
            await handle_csv_ai_lesson_tasks(self.bot, message, message.document)
        elif message.document.file_name == "code_tasks.csv":
            await handle_csv_code_tasks(self.bot, message, message.document)
        elif message.document.file_name != "lessons.csv" or "ai_test_tasks.csv":
            await self.bot.send_message(
                message.chat.id,
                "Please send the file in CSV format with the same name,\n" "as a table name in the DATABASE.",
            )
    ...
    async def handle_callback_query(self, call):
        ...
        await callback_query_handler(call, self.bot, self.inline_keyboard, self.reply_keyboard)

    async def display_current_mode(self, message):
        """
        ...
        if self.bot_processor.is_mode_main_menu():
            ...
            await self.bot.send_message(
                message.chat.id,
                "You are in check code mode.",
                reply_markup=self.inline_keyboard.get_keyboard(),
            )
            await check_code(message, self.bot)
            ...
            await process_lesson_topic(message, self.bot)
            ...
            await process_test_task_level(message, self.bot)
            ...
            await process_code_task_level(message, self.bot)
        elif self.bot_processor.is_mode_account():
            await self.bot.delete_message(message.chat.id, message.message_id)
            await self.bot.send_message(
                message.chat.id,
                "You are in account mode.",
                reply_markup=self.inline_keyboard.get_keyboard(),
            )
            await account_function(message, self.bot)
        elif self.bot_processor.is_mode_comments():
            await self.bot.send_message(
                message.chat.id,
                "You are in comments mode.",
                reply_markup=self.inline_keyboard.get_keyboard(),
            )
            await process_comments(self.bot, message)
        elif self.bot_processor.is_mode_help():
            await self.bot.send_message(
                message.chat.id,
                "You are in help mode.",
                reply_markup=self.inline_keyboard.get_keyboard(),
            )
            await help_request_handler(message, self.bot)
        elif self.bot_processor.is_mode_premium():
            await self.bot.send_message(
                message.chat.id,
                "You are in 'Premium' mode.",
                reply_markup=self.inline_keyboard.get_keyboard(),
            )
            await premium_status_function(message, self.bot)
