...
if __name__ == "__main__":
    ...
    # Create a new process to either create the file or read its content
    process = multiprocessing.Process(
        target=create_file_or_read_content,
        args=(ai_test_tasks_file_path, ai_test_tasks_prompt),
    )
    ...
    # Define test_tasks after the process has finished
    ...
    record_count = count_lines(ai_test_tasks_file_path)

    # Use function to define the prompt for generating responses
    prompt_test_tasks = prompt_test_task(test_tasks)

    # Continue generating responses until the record count reaches 300
    while record_count < 600:
        ...
        # Use functionn to create a set to store existing lines in the CSV file
        existing_lines = populate_existing_lines_set(ai_test_tasks_file_path)
        ...
        with open(ai_test_tasks_file_path, "a") as response_file:
            ...
            for line in response_lines:
                ...
                # Check if the length of the first field (question) is greater than 3 characters
                    ...
                    continue
                elif field[6] not in [field[3], field[5], field[4]]:
                    continue
                elif len({field[3], field[4], field[5]}) != 3:
                    continue
                # Check if the line is already present in the existing lines
                elif line.strip() in existing_lines:
